...
def train_model(test=False):
    data_module = BackgroundLeakDataModule(
        ...
        frames_per_video=200,  # Limit frames per video
        threshold=0.0,  # Threshold for similarity mask (LEAVE at 0.0 for continuous similarity measures, or set to a value like 0.6 for binary masks)
    )
    ...
    trainer.test(
        model,
        data_module,
        ckpt_path="lightning_logs/version_8/checkpoints/best-epoch=11-val_loss=-0.028-v1.ckpt",
    )
